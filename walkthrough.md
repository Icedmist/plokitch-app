...
* **Web Engine Asset Bundling Fix**: Declared the `.env` file explicitly in the `assets:` section of [pubspec.yaml](file:///home/adam/Projects/plokitch-app/pubspec.yaml) so that the Flutter Web engine compiles and packages the environment variables as a web-accessible asset, preventing the startup `NotInitializedError` from breaking Supabase.
...
### 4. Admin Role Privileges & View Switching
* **Action**: Promoted user to Admin and built a real-time UI/interface mode switcher.
* **Details**:
  * **Database Promotion**: Promoted `muhammadadamualiyu33@gmail.com` to the `'admin'` role directly in the PostgreSQL database schema.
  * **Admin View Controller Widget**: Integrated a control switcher widget right under the Profile Anchor Card on [settings_screen.dart](file:///home/adam/Projects/plokitch-app/lib/screens/settings_screen.dart). This widget is strictly visible only to authenticated `admin` accounts.
